...
def main(message):
    ...
    image_url = "./image/개인정보처리방침.png"
    # response = requests.get(image_url)
    
    # if response.status_code == 200:
    #     with open("./image/temp_image.png", "wb") as f:
    #         f.write(response.content)

    #     image = Image.open("./image/temp_image.png")
    ...
    if "agreed" in st.session_state and st.session_state.agreed:
        ...
        if st.button("전송", key="전송하기"):
            ...
            to_phone_number = st.text_input("수신자 전화번호", "+821071414807", help="예: +821012345678")
            message.send_twilio_message(message_body, to_phone_number)
